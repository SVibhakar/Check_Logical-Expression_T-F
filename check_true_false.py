...
def main(argv):
    ...
    for line in input_file:
        ...
        values = line.rstrip('\r\n').split()
        if (len(values) == 1):
           addl_symb[values[0]] = True
        else:
            if (len(values) == 2):
               temp = values[1][:-1]
               addl_symb[temp] = False
    ...
    r_kb = check_true_false(knowledge_base, statement, addl_symb)
    
    n_alpha = logical_expression()
    n_alpha.connective = ['not']
    n_alpha.subexpressions.append(statement)

    if (not valid_expression(n_alpha)):
        sys.exit('invalid statement')

    print ('\nChecking statement: ', print_expression(n_alpha, ''))

    r_alpha = check_true_false(knowledge_base, n_alpha, addl_symb)
    
    out_file = open("result.txt","wb")
    if (r_kb == True and r_alpha == True):
       print (" ")
       print ("both true and false ")
       out_file.write("both true and false")
    else:
       if (r_kb == True and r_alpha == False):
          print (" ")
          print ("definitely true")
          out_file.write("definitely true")
       else:
           if (r_kb == False and r_alpha == True):
              print (" ")
              ...
              out_file.write("possibly true, possibly false")
    out_file.close()
    sys.exit(1)
